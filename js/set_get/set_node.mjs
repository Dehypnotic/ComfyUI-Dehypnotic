// ╔═══════════════════════════════════════════════════════════════╗
// ║  Set Dehypnotic - virtual "named variable" sink               ║
// ╚═══════════════════════════════════════════════════════════════╝
//
...
// value back from anywhere with Get Dehypnotic - same value either way. Purely
...
const CATEGORY = "🧘 Dehypnotic/🔀 Wireless Links";

export function registerDehypnoticSetNode() {
  const LGraphNode = LiteGraph.LGraphNode;

  const oldNode = LiteGraph.registered_node_types[SET_TYPE];
  const displayTitle = oldNode?.title || "Set Dehypnotic";
  const displayCategory = oldNode?.category || CATEGORY;

  class DehypnoticSetNode extends LGraphNode {
    
    static title = displayTitle;
    static category = displayCategory;
    ...
    constructor(title) {
      ...
      // Brand dark defaults. Guarded so workflow-restore + right-click Colors still win.
    ...
    refreshTitle() {
      ...
      this.title = v ? displayTitle.replace("Set Dehypnotic", `Set: ${v}`) : displayTitle;
    }
    ...
    onConfigure() {
      ...
      // add it so older workflows gain the output.
    ...
    getExtraMenuOptions(_, options) {
      ...
      options.unshift({
        content: "Add paired Get Dehypnotic",
        callback: () => {
  ...
  LiteGraph.registerNodeType(SET_TYPE, DehypnoticSetNode);
}
