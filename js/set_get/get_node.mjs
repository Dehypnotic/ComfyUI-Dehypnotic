// ╔═══════════════════════════════════════════════════════════════╗
// ║  Get Dehypnotic - virtual "named variable" source             ║
// ╚═══════════════════════════════════════════════════════════════╝
//
// Minimal virtual node: a dropdown that picks which Set Dehypnotic to read (by
...
export function registerDehypnoticGetNode() {
  const LGraphNode = LiteGraph.LGraphNode;

  const oldNode = LiteGraph.registered_node_types[GET_TYPE];
  const displayTitle = oldNode?.title || "Get Dehypnotic";
  const displayCategory = oldNode?.category || CATEGORY;

  class DehypnoticGetNode extends LGraphNode {
    ...
    setType(type) {
     ...
     onRename() {
      ...
      if (setter) {
        ...
        this.title = displayTitle.replace("Get Dehypnotic", `Get: ${name}`);
      } else {
        this.setType("*");
        this.title = name ? displayTitle.replace("Get Dehypnotic", `Get: ${name}`) : displayTitle;
      }
  ...
  LiteGraph.registerNodeType(GET_TYPE, DehypnoticGetNode);
}
